...
export const getContratosCliente = async (clienteId) => {
  const { rows } = await pool.query('SELECT * FROM contratos ct WHERE ct.cliente_id = $1 ORDER BY ct.id DESC', [clienteId])
  return rows
}

export const getContratoDetalle = async (contratoId) => {
  const result = await pool.query('select ci.id, ci.servicio_id, ci.cantidad, ci.precio_unitario, ci.valoracion, ci.valoracion from contrato_items ci where contrato_id =$1', [contratoId])
  return result.rows
}
